...
// Telugu detection — supports Unicode Telugu script and Romanized Telugu (Tanglish)
...
const isQuestionAboutPavan = (input: string): boolean => {
  // Telugu Unicode messages are assumed to be about Pavan — pass them through
  if (TELUGU_UNICODE_REGEX.test(input)) return true;
...
Assistant: Busy ayithe busy! Empire build chesevaallu rest teesukuntaara ra? Software lo Baahubali anuko!`;
...
Assistant: Great question! Pavan is an AI engineer and builder -- he creates intelligent systems that make life smarter and work better.`;
  ...
  const languageInstruction = isUnicodeTelugu
    ? '\nIMPORTANT: The user wrote in Telugu script. Respond in Telugu script mixed with English where needed. Do not respond in English only.'
    : isTelugu
    ? '\nIMPORTANT: The user is writing in Romanized Telugu (Tanglish). Respond in Tanglish style as shown in examples.'
    : '';
